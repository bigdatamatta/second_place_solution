...
def make_image(img,size):
    ...
    return new_img
def extract_box(file_dir,dataset,file_name,idx,_id,box,img,features):
    (height, width,_) = img.shape
    [x,y,w,h] = box
    ...
    new_image = os.path.join(file_dir,"%s_%s_%s"%(dataset,_id,file_name))
...
def crop_image(file_dir,dataset,json_file):
    ...
    if isinstance(data['bounding_boxes'], dict):
        ...
        _id = data['bounding_boxes']['ID']        
        metadata_length = 47
        features = json_to_feature_vector(metadata_length, data, data['bounding_boxes'])
        extract_box(file_dir,dataset,file_name,0,_id,box,img,features)    
    else:
        for idx in range(len(data['bounding_boxes'])):
            box= data['bounding_boxes'][idx]['box']
            _id = data['bounding_boxes'][idx]['ID']        
            metadata_length = 47
            features = json_to_feature_vector(metadata_length, data,data['bounding_boxes'][idx])            
            extract_box(file_dir,dataset,file_name,idx,_id,box,img,features)
data_dir = sys.argv[1]#'test'
size = int(sys.argv[2])#280
dataset = 'test'#data_dir.split('/')[-1]
file_dir = './data/simplecut_%s_%s'%(dataset,size)
if not os.path.exists(file_dir):
    os.mkdir(file_dir)


files = glob.glob(data_dir+"/*/*_rgb.json")
print(len(files))
import tqdm
