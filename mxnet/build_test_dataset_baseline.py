...
count = 0 
def make_image(file_dir,dataset,file_name,idx,_id,box,img,size):
    global count 
    ...
    if r1 >= r2 or c1 >= c2:
        ...
        imgPath = os.path.join(file_dir,"%s_%d_%s"%(dataset,_id,file_name))
        subImg.save(imgPath)
...
def crop_image(file_dir,dataset,json_file):
    ...
    for idx in range(len(data['bounding_boxes'])):
        ...
        if box[2] <= 2 or box[3] <= 2:
            print(rgb_file)
            continue
        make_image(file_dir,dataset,file_name,idx,_id,box,img,size)

data_dir = sys.argv[1]#'test'
size = int(sys.argv[2])#299
dataset = 'test'#data_dir.split('/')[-1]
file_dir = './data/baseline_%s_%s'%(dataset,size)
