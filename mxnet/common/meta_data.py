...
def add_data_args(parser):
    ...
    data.add_argument('--data-train-mxnet', type=str, help='the training data')
    data.add_argument('--data-train-idx', type=str, default='', help='the index of training data')
    data.add_argument('--data-val', type=str, help='the validation data')
    data.add_argument('--data-val-mxnet', type=str, help='the validation data')
    ...
    data.add_argument('--image-shape-mxnet', type=str,
                      help='the image shape feed into the network, e.g. (3,224,224)')
...
def get_rec_iter(args, kv=None):
    ...
    image_shape_mxnet = tuple([int(l) for l in args.image_shape_mxnet.split(',')])
    ...
    train = mx.io.ImageRecordIter(
        ...
        num_parts           = nworker,
        part_index          = rank)
    train_mxnet = mx.io.ImageRecordIter(
        path_imgrec         = args.data_train_mxnet,
        path_imgidx         = args.data_train_idx,
        label_width         = 1,
        mean_r              = rgb_mean[0],
        mean_g              = rgb_mean[1],
        mean_b              = rgb_mean[2],
        data_name           = 'data_mxnet',
        #label_name          = 'softmax_label_mxnet',
        data_shape          = image_shape_mxnet,
        batch_size          = args.batch_size,
        rand_crop           = args.random_crop,
        max_random_scale    = args.max_random_scale,
        pad                 = args.pad_size,
        ...
        num_parts           = nworker,
        part_index          = rank) 
    meta_data = pd.read_csv('train_280_features.csv',header=None)
    meta_data = meta_data.values
    #print(meta_data.shape)
    meta_data =  meta_data / meta_data.max(axis=0)
    meta = mx.io.NDArrayIter(data={'meta_data':meta_data},batch_size=args.batch_size)
    train = mx.io.PrefetchingIter([train,train_mxnet,meta])
    ...
    val = mx.io.ImageRecordIter(
        ...
        num_parts           = nworker,
        part_index          = rank)
    val_mxnet = mx.io.ImageRecordIter(
        path_imgrec         = args.data_val_mxnet,
        path_imgidx         = args.data_val_idx,
        label_width         = 1,
        mean_r              = rgb_mean[0],
        mean_g              = rgb_mean[1],
        ...
        num_parts           = nworker,
        part_index          = rank)
    val_meta_data = pd.read_csv('val_280_features.csv',header=None)
    val_meta_data = val_meta_data.values
    val_meta_data = val_meta_data / val_meta_data.max(axis=0)
    val_meta = mx.io.NDArrayIter(data={'meta_data':val_meta_data},batch_size=args.batch_size)
    val = mx.io.PrefetchingIter([val,val_mxnet,val_meta])
    return (train, val)
