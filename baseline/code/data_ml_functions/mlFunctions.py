...
def get_cnn_model(params):   
    ...
    #baseModel = InceptionResNetV2(weights='imagenet',input_shape=(params.target_img_size[0], params.target_img_size[1], params.num_channels), include_top=False, input_tensor=input_tensor)
...
def img_metadata_generator(params, data, metadataStats,aug):
    """
    ...
    while True:
        for inds in batchInds:
            ...
            imgdata, metadata, labels = load_cnn_batch(params, batchData, metadataStats, executor,aug)
            if params.use_metadata:
                yield ([imgdata, metadata], labels)
            else:
                yield (imgdata, labels)

def load_cnn_batch(params, batchData, metadataStats, executor, aug):
    """
